...
(async () => {
  console.log('Launching Firefox for Remote Playwright Session...');
  const browser = await firefox.launch({ headless: true });
  
  const targetDir = 'test_suite/remote_session';
  if (!fs.existsSync(targetDir)){
      fs.mkdirSync(targetDir, { recursive: true });
  }

  for (let sessionIndex = 1; sessionIndex <= 1; sessionIndex++) {
      const pin = generatePin();
      console.log(`\n--- Starting Remote Session | PIN: ${pin} ---`);
      ...
      // Setup Session
      await gmPage.goto('https://zero-sum-rpg-2026.web.app/');
      ...
      const sessionDir = `${targetDir}/session_${pin}`;
